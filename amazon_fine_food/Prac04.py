...
"""
X 는 Text데이터
y 는 Text값에 의한 결과 값. 즉 0 혹은 1을 의미한다.
"""
#==============================================
#############  Score Prediction     ###############
# 로직에 의한 단어 카운트.
...
#================
"""
테스트 데이터의 양식을 확인하는 과정.
"""

def get_test(X, y, model,clf_model,coef_show=1):    
    X_c = model.fit_transform(X)
    print('# features: {}'.format(X_c.shape[1]))
    X_train, X_test, y_train, y_test = train_test_split(X_c, y, random_state=0)    
    # 트레이닝 데이터와 테스트 데이터로 나누는 과정이다.
...
get_test(X, y, c, LogisticRegression())
...
#===============
"""
일단 X_train, X_test, y_train, y_test = train_test_split(X_c, y, random_state=0)에서
반환되는 값들은 이미 여기에 주어지는 인자인 X_c로 인하여
X_test와 X_train의 값은 이미 모델화가 되어있어 사람이 보고 판단할 수 없다.
고로 만약 내가 만든 데이터를 테스트 데이터로 집어넣고 싶다면
동일한 과정으로 테스트 데이터를 만들어야 할 것 같다.
"""
...
#====================================
# 통합본에서 좋음과 나쁨의 단어들을 분류해서 추출.
# get_array_of를 통해서 평가의 통합본을 가져온다.
# 일반 워드카운트 : c = CountVectorizer(stop_words = 'english')
# TFIDF : tfidf = TfidfVectorizer(stop_words = 'english')
# TFIDF + Ngram : tfidf_n = TfidfVectorizer(ngram_range=(1,2),stop_words = 'english')
def get_array_of(X, y, model,clf_model,coef_show=1):
    X_c = model.fit_transform(X)
    X_train, X_test, y_train, y_test = train_test_split(X_c, y, random_state=0)
    clf = clf_model.fit(X_train, y_train)
    acc = clf.score(X_test, y_test)
    ...
    if coef_show == 1: 
        ...
        coeff_df = pd.DataFrame({'Word' : w, 'Coefficient' : coef})
        coeff_df = coeff_df.sort_values(['Coefficient', 'Word'], ascending=[0, 1])
        return coeff_df.head(20).to_string(index=False)+"\n"+coeff_df.tail(20).to_string(index=False)
all_df = get_array_of(X, y, tfidf,  LogisticRegression())
# all_df의 string형을 word와 coefficient를 분류한다.
print(all_df)
all_words = []
all_coeff = []
i = 0
for str in all_df.split():
  if i>1:
    if i%2 == 0:
      all_words.append(str)
    else:
      all_coeff.append(str)
  i = i+1
print("all words")
print(all_words)
print("all coeff")
print(all_coeff)
# word와 coefficient를 다시 좋음과 나쁨으로 나누어 준다.
good_words = []
good_coeff = []
bad_words = []
bad_coeff = []
for i in range(0,20):
  good_words.append(all_words[i])
  good_coeff.append(all_coeff[i])
print(good_words)
print(good_coeff)
for i in range(21,len(all_words)):
  bad_words.append(all_words[i])
  bad_coeff.append(all_coeff[i])
print(bad_words)
print(bad_coeff)
"""
